    ...
    public class GateFactory : IDataModelCommand
    {
        ...
        public bool BatchInsert(object[] items)
        {
            ...
            try
            {
                ...
                {
                    _db.AddObjectDirectly("tblGates", item);
                }
        ...
        public object[] GetUnSyncedItems()
        {
            ...
            {
                _db.Connection.Open();
                var lstUnSyncedItems = (from item in _db.tblGates
        ...
        public bool UpdatePatch(object[] items)
        {
            ...
            {
                _db.Connection.Open();
                var updateItems = items.OfType<tblGate>().ToList();

                foreach (var updateItem in updateItems)
                {
                    var item =
                        _db.tblGates.FirstOrDefault(
                            p => p.BranchId == updateItem.BranchId && p.GateCode == updateItem.GateCode);
                    item.IsSynced = true;
                }
                _db.SaveChanges();
                _db.Connection.Close();
